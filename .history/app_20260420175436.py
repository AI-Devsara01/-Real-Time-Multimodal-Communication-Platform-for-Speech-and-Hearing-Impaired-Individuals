...
for page in PAGES:
    def make_route(p):
        @app.route(f"/{p}", endpoint=p)
        def route_fn():
            return send_file(f"templates/{p}.html")
    make_route(page)
...
# ══════════════════════════════════════════════
#  IMAGE GENERATION API - Direct Python Implementation
# ══════════════════════════════════════════════
...
def api_image_gen():
    try:
        ...
        # Your exact working Python code
        ...
        # Make the request exactly like your Python code
        response = req.post(API_URL, headers=headers, json=data_payload)
        
        if response.status_code == 200:
            # Save image temporarily (just like your Python code)
            import tempfile
...
@app.route("/lip_sync")
def lip_sync_page():
    return send_file("templates/lip_sync.html")

@app.route("/mood_journal")
def mood_journal_page():
    return send_file("templates/mood_journal.html")
@app.route("/api/letter-recognition", methods=["POST"])
def api_letter_recognition():
    try:
        import requests
        import base64
        import tempfile
        ...
        if 'frame' not in request.files:
            return jsonify({"error": "No frame provided", "letter": "?", "confidence": 0.5}), 400
        
        frame_file = request.files['frame']
        
        # Save temporarily
        temp_path = tempfile.NamedTemporaryFile(delete=False, suffix='.jpg')
        frame_file.save(temp_path.name)
        ...
        if results.multi_hand_landmarks:
            ...
            FASTAPI_URL = "https://signbridge-production.up.railway.app/predict"
            response = requests.post(FASTAPI_URL, json={"landmarks": landmarks})
        ...
        return jsonify({"letter": "?", "confidence": 0.5})
        
    except Exception as e:
        print(f"Letter recognition error: {e}")
        return jsonify({"letter": "?", "confidence": 0.5, "error": str(e)}), 500
