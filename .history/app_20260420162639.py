...
@app.route("/api/image_gen", methods=["POST", "OPTIONS"])
def api_image_gen():
    ...
    try:
        ...
        # Your Cloudflare Worker details
...
# Alternative endpoint for direct image download
@app.route("/api/image_gen/download", methods=["POST"])
def api_image_gen_download():
    try:
        import requests as req
        data   = request.json
        prompt = data.get("prompt", "")
        ...
        if resp.status_code == 200:
            # Save to temp file and send as download
            ...
            return send_file(
                ...
                mimetype='image/jpeg',
                as_attachment=True,
                download_name=f"ai_image_{int(time.time())}.jpg"
            )
        else:
