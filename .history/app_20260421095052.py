...
// Replace the setInterval simulation with:
async function detectWord() {
    const canvas = document.getElementById('video-canvas');
    const frame = canvas.toDataURL('image/jpeg');
    
    const response = await fetch('/api/word-recognition', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ image: frame })
    });
    const data = await response.json();
    if (data.word && data.confidence > 0.7) {
        handleDetection(data.word, data.confidence);
    }
}
def load_users():
    global USERS
...
@app.route("/api/stt_server", methods=["POST"])
def api_stt_server():
    """Server-side speech recognition - Supports Indian languages better"""
    try:
        ...
        google_lang = lang_map.get(lang, 'en-IN')
